class Feed < ActiveRecord::Base
  ...
  has_many :user_feeds
  has_many :likers , :through => :user_feeds, :source => :user
  ...
  # def as_json(options, user_id)
  #   #call super(options)
  #   #x = has_favorited?(id)
  #   options.merge({include => entries, methods: => x} )
  # end
  #
  def has_favorited?(user_id)
    return self.likers.include?(User.find(user_id))
  end
end
